...
### pcap-test.c 추가 설명
1. header 위치 정하기
아래는 ethernet_header와 ipv4_header, tcp_header를 정의한 것을 의미한다. 
아래의 사진을 보면 각 데이터 버퍼가 ehternet, ip,tcp,payload가 하나의 버퍼로 이루어진 것을 볼 수 있다.
...
2. payload 데이터 뽑기
payload의 경우에는 앞서 했던 것처럼 현재 packet의 위치에서 각각의 header 길이만큼을 구해서 더하면 payload 시작 위치를 알 수 있다.
이를 이용해서 payload에 접근해 데이터를 뽑는 순서로 진행하였다.
내가 어려웠다고 생각하는 부분은 payload_length를 구하는 부분이다.
이 부분이 특히 어려웠던 건 header->caplen이 의미하는 바를 정확하게 이해하지 못하였기 때문이라고 생각한다.
결론은 header 구조체에는 caplen 과 len 이 있는데 caplen의 경우에는 읽어온 페이로드의 길이를 len은 페이로드 전체 길이를 의미한다.
따라서 내가 읽어온 페이로드의 길이에서 각각의 헤더를 빼주면 페이로드의 위치를 알 수 있게 된다.
```
